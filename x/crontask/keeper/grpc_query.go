...
func (q queryServer) TasksByAddress(ctx context.Context, req *crontasktypes.QueryTasksByAddressRequest) (*crontasktypes.QueryTasksResponse, error) {
	// Define a predicate function to filter by creator address
	predicateFunc := func(_ uint64, task crontasktypes.Task) (bool, error) {
		return task.Creator == req.Creator, nil
	}

	// Transform function to return task pointers
	transformFunc := func(_ uint64, task crontasktypes.Task) (*crontasktypes.Task, error) {
		return &task, nil
	}

	// Use filtered pagination on the main Tasks collection
	tasks, pageRes, err := query.CollectionFilteredPaginate(
		ctx,
		q.k.Tasks,
		req.Pagination,
		predicateFunc,
	...
	if err != nil && !errors.IsOf(err, collections.ErrInvalidIterator) {
		return nil, status.Error(codes.Internal, fmt.Sprintf("failed to paginate tasks: %v", err))
	}
...
func (q queryServer) TasksByStatusTimestamp(ctx context.Context, req *crontasktypes.QueryTasksByStatusTimestampRequest) (*crontasktypes.QueryTasksResponse, error) {
	...
	if !isValidStatus(req.Status) {
		return nil, status.Error(codes.InvalidArgument, fmt.Sprintf("invalid task status: %s", req.Status))
	}

	// Define a predicate function to filter by status
	predicateFunc := func(_ uint64, task crontasktypes.Task) (bool, error) {
		return task.Status == req.Status, nil
	}
