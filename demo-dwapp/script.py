...
def task(func):
    """
    Thin decorator that converts a function into a scheduled task.
    
    Extracts task-specific kwargs and creates a crontask to execute the function.
    Only the script address can call decorated functions.
    
    Task kwargs that get extracted:
    - task_scheduled_time: Unix timestamp when to execute
    - task_expiry_time: Unix timestamp when task expires  
    - task_gas_limit: Gas limit (default: 200000)
    - task_gas_fee_amount: Gas fee amount (default: 1)
    """
    def wrapper(*args, **kwargs):
        print(f"kwargs: {kwargs}")
        # Auth check - only script address can call
        if get_executor_address() != get_script_address():
            raise PermissionError(f"Only script address can call {func}")
        
        # If no scheduled time, execute immediately
        if not (task_scheduled_time := kwargs.get("task_scheduled_time")):
            return func(*args, **kwargs)
        
        script_addr = get_script_address()
        
        # Format scheduled timestamp - convert to string for Go parsing
        if isinstance(task_scheduled_time, int):
            scheduled_timestamp = str(task_scheduled_time)
        else:
            # Assume it's a duration string like "+1s", "+5m", etc.
            scheduled_timestamp = str(task_scheduled_time)
        
        # Format expiry timestamp 
        if not (task_expiry_time := kwargs.get("task_expiry_time")):
            # Leave empty string to use default
            expiry_timestamp = ""
        elif isinstance(task_expiry_time, int):
            expiry_timestamp = str(task_expiry_time)
        else:
            expiry_timestamp = str(task_expiry_time)
        ...
        return _msg({
            ...
            "task_gas_limit": str(kwargs.get("task_gas_limit", 500000)),
            "task_gas_fee":  {"denom": "dys", "amount": kwargs.get("task_gas_fee_amount", "1")},
            "msgs": [{
                "@type": "/dysonprotocol.script.v1.MsgExec",
                "executor_address": script_addr,
                "script_address": script_addr,
                "function_name": str(func).split(" ")[1].strip("<>"), # We do not have access to dunder methods in script
...
@task
def task_fib(n: int = 3,  **kwargs):
    print(f"task_fib(n={n})")
    if n == 0:
        return 0
    if n == 1:
        return 1
    return task_fib(n - 1, **kwargs) , task_fib(n - 2, **kwargs)
