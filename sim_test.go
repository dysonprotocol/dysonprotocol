...
func TestFullAppSimulation(t *testing.T) {
	simtestutil.Run(t, NewSimApp, setupStateFactory)
}

func setupStateFactory(app *SimApp) simtestutil.SimStateFactory {
	return simtestutil.SimStateFactory{
		Codec:         app.AppCodec(),
...
func TestAppImportExport(t *testing.T) {
	simtestutil.Run(t, NewSimApp, setupStateFactory, func(tb testing.TB, ti simtestutil.TestInstance[*SimApp], accs []simtypes.Account) {
		tb.Helper()
		app := ti.App
		...
		newTestInstance := simtestutil.NewSimulationAppInstance(tb, ti.Cfg, NewSimApp)
...
func TestAppSimulationAfterImport(t *testing.T) {
	simtestutil.Run(t, NewSimApp, setupStateFactory, func(tb testing.TB, ti simtestutil.TestInstance[*SimApp], accs []simtypes.Account) {
		...
		_, err = newApp.InitChain(&abci.RequestInitChain{
			...
			ChainId:       simtestutil.SimAppChainID,
		})
		if IsEmptyValidatorSetErr(err) {
			tb.Skip("Skipping simulation as all validators have been unbonded")
			return
		...
		_, _, err = simulation.SimulateFromSeedX(
			...
			simtestutil.WriteToDebugLog(newTestInstance.AppLogger),
			newApp.BaseApp,
...
func TestAppStateDeterminism(t *testing.T) {
	...
	captureAndCheckHash := func(tb testing.TB, ti simtestutil.TestInstance[*SimApp], _ []simtypes.Account) {
		tb.Helper()
	...
	// run simulations
	simtestutil.RunWithSeeds(t, interBlockCachingAppFactory, setupStateFactory, seeds, []byte{}, captureAndCheckHash)
}
