...
// Profile with:
// /usr/local/go/bin/go test -benchmem -run=^$ cosmossdk.io/dysapp -bench ^BenchmarkFullAppSimulation$ -Commit=true -cpuprofile cpu.out
func BenchmarkFullAppSimulation(b *testing.B) {
	b.ReportAllocs()

	config := simcli.NewConfigFromFlags()
	config.ChainID = simtestutil.DysAppChainID
	...
	app := NewDysApp(logger, db, nil, true, appOptions, interBlockCacheOpt(), baseapp.SetChainID(simtestutil.DysAppChainID))
